...
public:
	...
	virtual	void Activate( const char * input ) {
		...
		if (!_stricmp(input, "regedit")) {
			cNetwork::Shell_Command("regedit");
		} else if (!_stricmp(input, "logfile")) {
...
public:
   ...
   virtual	const char * Get_Alias( void ) { return "rm"; }
	...
	virtual	void Activate( const char * input ) {
		if (COMBAT_SCENE != NULL) {
			if ((_stricmp(input,"disable") == 0) || (_stricmp(input,"0") == 0)) {
			...
			} else if ((_stricmp(input,"zbuffered") == 0) || (_stricmp(input,"2") == 0)) {
...
public:
   ...
   const char * Get_Alias( void ) { return "dt"; }
	...
	virtual	void Activate( const char * input ) {
		...
		} else if (_stricmp(input,"warning") == 0) {
		...
		} else if (_stricmp(input,"error") == 0) {
		...
		} else if (_stricmp(input,"script") == 0) {
		...
		} else if (_stricmp(input,"netadmin") == 0) {
		...
		} else if (_stricmp(input,"netbasic") == 0) {
		...
		} else if (_stricmp(input,"netprolific") == 0) {
...
public:
	...
	virtual void Activate(const char * input) {
		if (!_stricmp(input, "success")) {
			CombatManager::Mission_Complete(true);
		} else if (!_stricmp(input, "failure")) {
...
public:
   ...
   const char * Get_Alias( void ) { return "dd"; }
	...
	virtual	void Activate( const char * input ) {
		...
		} else if (_stricmp(input,"dbwin32") == 0) {
		...
		} else if (_stricmp(input,"windows") == 0) {
	...
	bool KickGameSpyUser(const WideStringClass & user_name) {
		if (cGameSpyAdmin::Is_Gamespy_Game()) {
			...
			for (SLNode<cPlayer> *player_node = cPlayerManager::Get_Player_Object_List ()->Head ()
				...
				if (_stricmp(StringClass(user_name), StringClass(player->Get_Name())) == 0) {
	...
	virtual	void Activate( const char * input ) {
		...
		if ((cGameSpyAdmin::Is_Gamespy_Game() || is_wol) && cNetwork::I_Am_Server()) {
			...
			for (SLNode<cPlayer> *player_node = cPlayerManager::Get_Player_Object_List ()->Head ()
				...
				if (_stricmp(StringClass(input), StringClass(player->Get_Name())) == 0) {
					if (is_wol) {
...
void	ConsoleFunctionManager::Sort_Function_List( void )
{
	...
	while ( !done ) {
		...
		for (	int index = 0; index < count; index++) {
			if ( _stricmp(	FunctionList[index]->Get_Name(),
	...
	if ( ( function_name == NULL ) || ( function_name[0] == 0 ) ) {
      ...
      Print( "------ Available commands -------\n" );
		*string = 0;
		...
		for (	int index = 0; index < FunctionList.Count(); index++) {
			ConsoleFunctionClass * function = FunctionList[index];
			if ( !_stricmp( function->Get_Name(), function_name ) ) {
...
{
	if (!_strnicmp(string, "help", 4)) {
		Help( string + 4 );
	} else {
		for (	int index = 0; index < FunctionList.Count(); index++) {
			ConsoleFunctionClass * function = FunctionList[index];

         if ( !_strnicmp( string, function->Get_Name(), strlen( function->Get_Name() ) ) ) {
         ...
         if (function->Get_Alias() != NULL &&
            ...
            if (!_strnicmp(string, alias, alias_len) &&
...
int ConsoleFunctionManager::Find_Function_Node(const char * name)
{
	for (	int index = 0; index < FunctionList.Count(); index++) {
		ConsoleFunctionClass * function = FunctionList[index];
		if (_stricmp(name,function->Get_Name()) == 0) {
			return index;
		}
	}
	return -1;
