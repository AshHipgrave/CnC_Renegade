	...
	for (int adapter_index=0; adapter_index<adapter_count; adapter_index++) 
	{
		...
		if (res == D3D_OK) {
			...
			*/
			...
			res = D3DInterface->GetAdapterDisplayMode(adapter_index, &d3dmode);

			if (res != D3D_OK)
			{
				WWDEBUG_SAY(("Failed to get DisplayMode for adaptor %d. It will be skipped.", adapter_index));
				continue;
			}
			...
			*/
			...
			int mode_count = D3DInterface->GetAdapterModeCount(adapter_index, d3dmode.Format);
			for (int mode_index=0; mode_index<mode_count; mode_index++) {				
				
				res = D3DInterface->EnumAdapterModes(adapter_index, d3dmode.Format, mode_index, &d3dmode);
...
bool DX8Wrapper::Find_Color_Mode(D3DFORMAT colorbuffer, int resx, int resy, UINT *mode)
{
	...
	HRESULT hr = D3DInterface->GetAdapterDisplayMode(D3DADAPTER_DEFAULT, &dmode);
	WWASSERT(hr == D3D_OK);
	...
	unsigned short vertex_count)
{
	...
	// TODO: AshHipgrave - Confirm if this sequence is correct:
...
void DX8Wrapper::Set_DX8_NPatch_Mode(const float segments)
{
    DX8CALL(SetNPatchMode(segments));
    DX8_RECORD_RENDER_STATE_CHANGE();
}

void DX8Wrapper::Set_DX8_Sampler_State(const DWORD sampler, const D3DSAMPLERSTATETYPE type, const DWORD value)
{
    DX8CALL(SetSamplerState(sampler, type, value));
    DX8_RECORD_RENDER_STATE_CHANGE();
}
