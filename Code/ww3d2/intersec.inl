...
*/
inline bool IntersectionClass::Intersect_Polygon_Z(IntersectionResultClass *InResult, Vector3 &PolygonNormal, Vector3 &v1, Vector3 &v2, Vector3 &v3)
{

	InResult->Range = Plane_Z_Distance(PolygonNormal, v1);
	(InResult->Intersection)[0] = (*RayLocation)[0];
	(InResult->Intersection)[1] = (*RayLocation)[1];
	(InResult->Intersection)[2] = (*RayLocation)[2] - InResult->Range;
	return _Point_In_Polygon(InResult, PolygonNormal, v1, v2, v3);
}
...
*/
void IntersectionClass::Calculate_Intersection(IntersectionResultClass *InResult)
{
	(InResult->Intersection)[0] = (*RayLocation)[0] + (*RayDirection)[0] * InResult->Range;
	(InResult->Intersection)[1] = (*RayLocation)[1] + (*RayDirection)[1] * InResult->Range;
	(InResult->Intersection)[2] = (*RayLocation)[2] + (*RayDirection)[2] * InResult->Range;
}
...
*/
inline bool IntersectionClass::Intersect_Plane_Quick(IntersectionResultClass * InResult, Vector3 &PlaneNormal, Vector3 &PlanePoint)
{
	// do a parallel check
	float divisor = (PlaneNormal[0] *(*RayDirection)[0] + PlaneNormal[1] *(*RayDirection)[1] + PlaneNormal[2] * (*RayDirection)[2]);
	if(divisor == 0) return false; // parallel
	...
	InResult->Range = - (d + PlaneNormal[0] * (*RayLocation)[0] + PlaneNormal[1] *  (*RayLocation)[1] + PlaneNormal[2] *  (*RayLocation)[2])  / divisor;
...
*/
inline bool IntersectionClass::Intersect_Plane(IntersectionResultClass * InResult, Vector3 &PlaneNormal, Vector3 &PlanePoint) {
								 
	// normalize the ray direction
	RayDirection->Normalize();

	// call the quick test routine
	if(!Intersect_Plane_Quick(InResult, PlaneNormal, PlanePoint)) return false;

	// check to make sure it's not behind the ray's origin
	if(InResult->Range <= 0) return false;

	// check to make sure it's not beyond max distance
	if(InResult->Range > MaxDistance) return false;

	// determine point of intersection
	Calculate_Intersection(InResult);
...
*/
inline bool IntersectionClass::Intersect_Polygon(IntersectionResultClass * InResult, Vector3 &PolygonNormal, Vector3 &v1, Vector3 &v2, Vector3 &v3)
{
	...
	if(Intersect_Plane(InResult, PolygonNormal, v1)) {
...
*/
inline bool IntersectionClass::Intersect_Polygon(IntersectionResultClass * InResult, Vector3 &v1, Vector3 &v2, Vector3 &v3)
{
	Vector3 vec1 = v2 - v1;
	Vector3 vec2 = v3 - v1;
	Vector3 normal = Vector3::Cross_Product(vec1, vec2);

	return Intersect_Polygon(InResult, normal, v1,v2,v3);
}
