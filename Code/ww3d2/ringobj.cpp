...
void RingRenderObjClass::render_ring(RenderInfoClass & rinfo,const Vector3 & center,const Vector3 & extent)
{
	...
	DynamicVBAccessClass vb(BUFFER_TYPE_DYNAMIC_SORTING,dynamic_fvf_type,ring.Vertex_ct);
	{
		...
		VertexFormatXYZNDUV2 *vba = Lock.Get_Formatted_Vertex_Array();
		...
		{			
			...
			vba->nx = ring.vtx_normal[i].X;		// may not need this!
			vba->ny = ring.vtx_normal[i].Y;
			vba->nz = ring.vtx_normal[i].Z;

			vba->diffuse = color;
			
			if (RingTexture) {
				vba->u1 = ring.vtx_uv[i].X;
				vba->v1 = ring.vtx_uv[i].Y;
			}
			vba++;
		}		
	}
