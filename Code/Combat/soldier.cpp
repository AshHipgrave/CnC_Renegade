		...
		for (int index = 0; index < obj_count; index ++) {
			...
			if (render_obj != NULL) {
				...
				if ( htree != NULL ) {
					...
					//
					if ( ::_stricmp (htree->Get_Name(), "S_A_HEAD") == 0 ||
						  ::_stricmp (htree->Get_Name(), "S_B_HEAD") == 0)
					{
		...
		for ( int emitter = 0; emitter < NUM_SPECIAL_DAMAGE_EMITTERS; emitter++ ) {
			if ( _SpecialDamageEmitters[emitter].Mode == SpecialDamageMode ) {
				...
				while ( --sub_count >= 0 ) {
					...
					if ( ::_stricmp(sub->Get_Name(), _SpecialDamageEmitters[emitter].EmitterName ) == 0 ) {
...
{
	for ( int i = 0; i < RenderObjList.Count(); i++ ) {
		if ( !_stricmp( RenderObjList[i]->Get_Name(), name ) ) {
